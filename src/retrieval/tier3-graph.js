...
export function tier3(state, seeds, queryStr, intent, ctx) {
    ...
    for (let hop = 1; hop <= RETRIEVAL.TIER3_MAX_HOPS; hop++) {
        ...
        for (const item of frontier) {
            for (const edge of neighborsOf(adjacency, item.entry.id)) {
                ...
                const beamScore = Math.exp(RETRIEVAL.TIER3_LAMBDA_1 * edgeWeight + RETRIEVAL.TIER3_LAMBDA_2 * bm25);
                candidates.push({ entry: neighbor, beamScore, hop });
            }
        }
        if (candidates.length === 0) break;
        candidates.sort((a, b) => b.beamScore - a.beamScore);
        const topOfHop = candidates.slice(0, RETRIEVAL.TIER3_BEAM_WIDTH);

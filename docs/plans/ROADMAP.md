...
**What shipped:** Public-ship release of STARmem v2.0.0 at commit `5195b2e`, tag `2.0.0` on `origin`. Quiet Library visual identity (token system: typography / spacing / elevation / radius / motion; serif display face; staged 600ms viewer entrance reveal; reduced-motion respected). Three structural survivors from Phase 15: T2 (Traces consolidation events with shape `{kind: 'consolidate', timestamp, chatId, summary, durationMs, extractor}`), T3 (tier-label honesty—T0/T1/T3/Floor; legacy T2 backfilled to T3 since seed-into-Tier-3 is what historical Tier 2 hits exercised), T8 (viewer ARIA + keyboard navigation: tablist/tab/tabpanel wired; roving tabindex; Arrow/Home/End/Enter handlers). Three cosmetic successors under per-task eyeball gates: T5 (consolidation indicator—accent pulse + responsive anchor), T6 (episodic tab—subject groups with display headings + dotted separators), T7 (settings panel—display-font sections + 220px-label grid). Public-ship pass at T9: manifest `2.0.0-dev` → `2.0.0`, README rewrite (option-A blockquote leading with "no LLM on the query path") + screenshot + install block + citations footer, `CHANGELOG.md` from scratch summarizing Phases 0–16, `docs/install.md`, `docs/screenshots/viewer.png`, annotated tag `2.0.0` pushed to `origin`. Plus T0 (plan + Phase-15 retro amendment) and T0.5 (extension-install conventions reference doc).

**Test totals:** Entry 98 suites / 974 tests (jest, from Phase 15 close). Close: **105 suites / 1010 tests (+7 / +36)**, all green. 58 (pytest bench/modal, unchanged), 9 (Playwright E2E, unchanged). New suites this phase: T2 trace shape, T3 tier labels, T6 episodic grouping, T8 ARIA, T8 keyboard, plus minor companions. Lint 0 errors / 10 warnings (baseline parity). Typecheck has 2 P16-introduced JSDoc-narrowness errors filed as v2.1 candidates (Trace typedef widening for consolidation kind; HTMLInputElement cast in episodic-grouping test); ~14 other typecheck errors are pre-existing `bench/` typedef gaps.

**Commits this phase:** ~12 total. Plan + Phase-15 amendment (`2ecbf02`) → install conventions reference (`37d63b0`, `fa44a26`) → preference application to plan (`4e6cb50`) → identity mockup (`2b949b1`) → T2 trace events (`3c8d939`) → T3 tier labels (`660ddca`) → T4 Quiet Library tokens + viewer shell (`3450916`, `80eb017`) → T5 indicator (`bf49233`) → T6 episodic + scroll-chain followup (`eae87da`, `59f48aa`) → T7 settings + em-dash followup (`93f0cd1`, `413ca97`) → T8 ARIA + keyboard (`55ebf67`) → T9 public-ship release (`5195b2e`) → T10 retro + ROADMAP (this commit). Phase 16 range: `2ecbf02..HEAD`.

**Execution mode:** Strict serial. Controller-only end-to-end; the plan-header decision tagging some tasks as subagent-friendly (`[structural]`) and others as hybrid (`[cosmetic]` controller-drafts-then-dispatches) collapsed in practice—task surfaces were small enough that direct execution beat dispatch overhead, matching the Phase 15 lesson on T1. Per-task acceptance tags (`[structural] / [cosmetic] / [doc] / [gate] / [public-ship]`) drove verdict shape: `[structural]` shipped on jest-green; `[cosmetic]` shipped on Eva-eyeball verdict; `[doc]` and `[public-ship]` on internal review. The Phase 15 lesson 3.1 collapse shape did not recur.

**Decisions held / revised:** 13 of 14 plan-header decisions held (strict serial; per-task acceptance tags; Quiet Library identity; single token system; eyeball-as-gate; reference-extension calibration; `frontend-design` skill use; three structural survivors; cosmetic successors under new identity; preflight-audit per task—N/A under controller-only; phase-15 amendment as precondition; manifest as single source of truth for v2.0; surface-specific copy for repo description vs. README blockquote). One revised: hybrid execution mode collapsed to controller-only—calibration data, not framing failure. See `phase-16-retro.md` §2 for the full table.

**Surprises:**

1. **Eyeball-as-gate promoted from speculation to confirmed primitive.** Phase 15 retro §3.2 hypothesized the rule (gate, not smoke). Phase 16 ran the experiment: T1 (identity mockup) gated the cosmetic phase pre-implementation; per-task eyeball verdicts at each cosmetic commit caught two in-flight followups (`59f48aa` scroll-chain bounce, `413ca97` em-dash) and zero rescopes. The pattern works.
2. **`frontend-design` skill's identity-language layer translated cleanly; CSS-cascade specifics needed manual fix-up at host integration.** T4's commit explicitly notes "ST popup cascade fixes" alongside the Quiet Library application. Datapoint for the skill: when applying its output inside an embedded host, plan additional time for cascade-conflict resolution, or add a "verify against host CSS in a live mock" step between ideation and implementation. Two-case pattern now (Phase 15 `--starmem-overlay` + Phase 16 cascade fixes); the skill patch is on file but defer until the third confirming case.
3. **Repo description and README blockquote are different surfaces with different audiences.** Eva's mid-T9 call to author the `gh repo edit` description independently from the README blockquote (option A) is a public-ship convention worth keeping: short keyword-dense one-liner for GitHub-list discoverability, principle-led three-line blockquote for landing-page first impression. Bundling them flattens both.
4. **The cosmetic eyeball gate caught a structural integration issue (T6 scroll-chain bounce).** Cosmetic work and structural integration aren't always cleanly separable; the eyeball gate is the right time to catch the joint. Worth noting as a pattern for future cosmetic-heavy work in host-embedded contexts.

**Notes for v2.1+:**

- Pixel-snapshot tests, `prefers-reduced-motion` test coverage, and a light/high-contrast theme matrix all surfaced as v2.1 candidates. Defer until a regression actually escapes the eyeball gate or until a11y / cross-theme robustness becomes an explicit project value commitment.
- House-style grep guard for em dashes in user-facing strings would catch the T7 slip class automatically; companion to existing CSS-hygiene invariants.
- Two P16-introduced typecheck errors (Trace typedef widening for consolidation kind; HTMLInputElement cast) belong in a v2.1 typecheck-hygiene cleanup. Mechanical; not blocking.
- Public-ship phase shape now codified: tag and release-body authoring should split into `[public-ship-tag]` and `[public-ship-release]` tasks. Phase 16 collapsed them in the plan; in practice Eva separated the release-body authoring outside the controller loop.
- Install conventions reference doc (T0.5, `docs/references/extension-install-conventions.md`) is reusable for future ST extensions and removes the rederivation cost.
- The deterministic-retrieval principle, single-substrate context tree, scope vocabulary (Working / Episodic / Persona), edge-type set, 4-tier ladder, and latency budgets all preserved end-to-end through the v2.0 cycle.

**v2.1 candidates filed:** Pixel-snapshot tests; `prefers-reduced-motion` coverage; light/high-contrast theme matrix; em-dash grep guard; Trace typedef widening; HTMLInputElement cast cleanup; tag-then-release public-ship split. **Skill patch candidates:** `frontend-design` host-cascade verification step (second confirming case on file, defer to third).

**Phase 16 closes the v2.0 cycle.** STARmem is publicly shipped at v2.0.0—commit `5195b2e`, tag `2.0.0` on `origin`. GitHub release publication and `gh repo edit` description are Eva-deferred outside the controller loop. The next phase is v2.1, scope TBD.

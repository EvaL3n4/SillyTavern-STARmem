...
3. **Consolidation sweep adds `BATCH_SIZE`.** 9.4.7 revealed BATCH_SIZE is non-cosmetic (a single idle consolidation drained only 5 of 9 residual entries, required while-loop). Sweep values: `[3, 5, 10, 15]`. Ships alongside `DEDUP_JACCARD_THRESHOLD`. Two rounds, ~9 points total.
...
7. **Scope-locked preflight fixes** — two source-code issues that block sweeping cleanly must land in 9.4.9 Task 1 before any sweep runs:
   - `BATCH_SIZE` is NOT in `_SWEPT_CONSOLIDATION_KEYS` — adds.
   - `TIER3_SEEDS_K` is NOT in `_SWEPT_RETRIEVAL_KEYS` — adds.
   - `src/consolidation/consolidate.js:47` destructures `BATCH_SIZE` at module top (**writing-plans preflight trap**) — rewrites to `CONSOLIDATION.BATCH_SIZE` at call time inside `consolidate()`.
